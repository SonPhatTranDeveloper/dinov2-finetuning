...
class TrainerWeightedInterval:
    def __init__(self, model, device, train_loader, val_loader, args,
                 start_weight=0.1, end_weight=0.9, interval=10):
        """
        Initialize the trainer for the DINOv2 ViT result
        """
        # Cache the parameters
        ...
        self.step = (end_weight - start_weight) / (args["epochs"] / interval)
    ...
    def train(self, epoch):
        ...
        :return: epoch loss and accuracy
        """
        # Adjust the weight of the Sinkhorn normalization
        if epoch % self.interval == 0:
            self.current_weight += self.step
            self.model.adjust_sinkhorn_weight(self.current_weight)
